...
def initialize_tree(data, cell_stages, rigorous_gap_stats = False, min_split=15, min_percentage_split=0.25):
    ...
    cluster_IDs_per_stage[0].extend([k for k in range(N_best_clusters)])
    ...
    for stage_idx in range(1, N_stages):
        ...
        for j in range(N_previous_clusters):
...
def refine_tree(data, centroid_coordinates, cluster_indices, parent_clusters, cell_stages, 
    ...
    N_clusters_per_stage = np.array([len(v) for v in list(parent_clusters.values())], dtype = int)
    ...
    while iter < max_iter:
        ...
        for k, v in list(parent_clusters.items()):
        ...
        delta_parents = 0
        for v1, v2 in zip(list(outdated_parent_clusters.values()), list(parent_clusters.values())):
    ...
    with open(final_tree_path, 'w') as f:
        ...
        f.write('\t'.join(['MU {0}'.format(k + 1) for k in range(N_features)]))
        f.write('\n')
...
def bifurcation_direction(data, cell_IDs, markers, parent_clusters, centroid_coordinates,
    ...
    for stage_idx, parents in list(parent_clusters.items()):
        if stage_idx > 0:
    ...
    header = ['Stage']
    for k, v in list(bifurcation_info.items()):
...
def bifurcation_analysis(cluster_indices, bifurcation_info, bifurcation_directions,
    ...
    counter = 0
    for stage, vectors in sorted(list(bifurcation_info.items()), key = lambda tpl: tpl[0]):
        for v in vectors:
...
def get_ordinals(N):
    ...
    ordinals.extend(['{0}th'.format(i) for i in range(4, N + 1)])
