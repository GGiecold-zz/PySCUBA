...
def reduction_simulations(projected_data_per_split, parameters_per_split, ):
    """Predict the effects of perturbing potential regulators on the lineage bias.
    """
    
    reduction_powers = [1, np.log2(10), np.log2(100)]
    
    limit = int(floor(1.5 * np.max(np.abs(projected_data[-1]))))
    grid = np.linspace(-limit, limit, num = 5000)
          
    roots = np.roots([-1, 0, parameters[-1], parameters[1]])
    roots = np.sort(np.real(roots))
    
    unstable_root = roots[1]
    left_well_position = roots[0]
    right_well_position = roots[-1]
    
    left_well_idx = np.where(grid < left_well_position)[0][-1]
    right_well_idx = np.where(grid >= right_well_position)[0][0]
    
    grandpa_pot = [potential(x, parameters[0], parameters[1], parameters[2]) for x in grid]
    parent_pot = [potential(x, parameters[0], parameters[1], parameters[3]) for x in grid]
    children_pot = [potential(x, parameters[0], parameters[1], parameters[4]) for x in grid]
    
    exp_potential = np.exp(children_pot[left_well_idx:right_well_idx])
    
    split_prob_normalization = np.sum(exp_potential) * (grid[1] - grid[0])
    
    #split_prob_right = np.cumsum(np.exp(children_pot[left_well_idx:right_well_idx])) *
    #limit = int(floor(1.5 * np.max(np.abs(data[-1]))))
    #likelihood = 1.0
    #for distro in multistate_distribution(data, parameters, limit, False):
        #likelihood *= reduce(operator.mul, (10 ** 2) * distro, 1.0)  
    
    return mutant_splits
